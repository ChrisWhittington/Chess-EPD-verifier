...
// set true/false depending whether you want to test the code, or test some epds
#define TEST_ME_TO_DESTRUCTION TRUE

#if TEST_ME_TO_DESTRUCTION
...
#define CORNERS_BB (0x8100000000000081)
...
u64 Wsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 Esmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 Nsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 Ssmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 NWsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 SWsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 NEsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 SEsmearuntilnotempty(u64 bb, u64 occupiers)
{
	u64 smear = 0;
	do
	{
...
u64 SEsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 SWsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 NEsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 NWsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 Nsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 Ssmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 Esmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 Wsmearmove(u64 bb, u64 all_us, u64 all_them)
{
	u64 moves = 0;
	do
	{
...
u64 bitboard_bishop_attack_smear(u64 us, u64 occ)
{
...
u64 bitboard_rook_attack_smear(u64 us, u64 occ)
{
...
u64 bitboard_queen_attack_smear(u64 us, u64 occ)
{
	u64 bb = NWsmearuntilnotempty(us, occ);
	bb |= NEsmearuntilnotempty(us, occ);
	bb |= SWsmearuntilnotempty(us, occ);
...
// returns 1 if true
bool IsSquareAttacked(int us, int s1, u64 ourpieces, u64 opp_pawns, u64 opp_nites, u64 opp_bishops, u64 opp_rooks, u64 opp_queens, u64 opp_king)
{
	u64 bb = (ONE << s1);
	if (KINGSMEAR(bb) & opp_king) return 1;
	if (NITESMEAR(bb) & opp_nites) return 1;
	if (PAWNSCAPTURE(bb, us) & opp_pawns) return 1;
	u64 occupied = ourpieces | opp_pawns | opp_nites | opp_bishops | opp_rooks | opp_queens | opp_king;
	if (bitboard_bishop_attack_smear(bb, occupied) & (opp_bishops | opp_queens)) return 1;
	if (bitboard_rook_attack_smear(bb, occupied) & (opp_rooks | opp_queens)) return 1;
	return 0;
}
//
...
//
// returns 1 if true
bool IsGiveCheck(GAMEDATA* gd, int us)
{
	int them = us ^ 1;

	u64 opppieces = gd->pawns[them] | gd->nites[them] | gd->bishops[them] | gd->rooks[them] | gd->queens[them] | gd->king[them];
	u64 our_pawns = gd->pawns[us];
	u64 our_nites = gd->nites[us];
	u64 our_bishops = gd->bishops[us];
	u64 our_rooks = gd->rooks[us];
	u64 our_queens = gd->queens[us];
	u64 our_king = gd->king[us];

	int s1 = scanforward64(gd->king[them]);
	return (IsSquareAttacked(them, s1, opppieces, our_pawns, our_nites, our_bishops, our_rooks, our_queens, our_king));
}
//
...
int ep_string_to_square(const char *str)
{
	...
	return (square_from(str[1] - '1', str[0] - 'a'));
}
//

void set_square_to_bb(GAMEDATA* gd, char ch, int rank, int file)
{
	int index;
	index = (rank << 3) + file;
	switch (ch)
	{
	case 'p':
		gd->pawns[1] |= (ONE << (index));
		break;
	case 'P':
		gd->pawns[0] |= (ONE << (index));
		break;
	case 'n':
		gd->nites[1] |= (ONE << (index));
		break;
	case 'N':
		gd->nites[0] |= (ONE << (index));
		break;
	case 'b':
		gd->bishops[1] |= (ONE << (index));
		break;
	case 'B':
		gd->bishops[0] |= (ONE << (index));
		break;
	case 'r':
		gd->rooks[1] |= (ONE << (index));
		break;
	case 'R':
		gd->rooks[0] |= (ONE << (index));
		break;
	case 'q':
		gd->queens[1] |= (ONE << (index));
		break;
	case 'Q':
		gd->queens[0] |= (ONE << (index));
		break;
	case 'k':
		gd->king[1] |= (ONE << (index));
		break;
...
char* LoadFEN(GAMEDATA* gd, char* fen)
{
	...
	char* token = strtok_s(fen, " ", &rest_of_epd_str);
	...
	while ((ch = *token++))
	{
		...
		else
		{
			...
			else
			{
				...
				set_square_to_bb(gd, ch, rank, file);
				file++;
			}
		}
	}
	...
	if (((gd->pawns[0] | gd->pawns[1])) & (H1 | H8))
	{
		return 0;
	}
	...
	u64 all_w = gd->pawns[0] | gd->nites[0] | gd->bishops[0] | gd->rooks[0] | gd->queens[0];
	u64 all_b = gd->pawns[1] | gd->nites[1] | gd->bishops[1] | gd->rooks[1] | gd->queens[1];
	if (popcount64(all_w) > 15) return 0;
	if (popcount64(all_b) > 15) return 0;

	if (popcount64(gd->king[0]) != 1) return 0;
	if (popcount64(gd->king[1]) != 1) return 0;

	int n_wp = popcount64(gd->pawns[0]);
	int n_bp = popcount64(gd->pawns[1]);
	if ((n_wp > 8) || (n_bp > 8)) return 0;

	// white piece counts
	if (popcount64(gd->nites[0]) > (2 + (8 - n_wp))) return 0;
	if (popcount64(gd->bishops[0]) > (2 + (8 - n_wp))) return 0;
	if (popcount64(gd->rooks[0]) > (2 + (8 - n_wp))) return 0;
	if (popcount64(gd->queens[0]) > (1 + (8 - n_wp))) return 0;
	// again for black
	if (popcount64(gd->nites[1]) > (2 + (8 - n_bp))) return 0;
	if (popcount64(gd->bishops[1]) > (2 + (8 - n_bp))) return 0;
	if (popcount64(gd->rooks[1]) > (2 + (8 - n_bp))) return 0;
	if (popcount64(gd->queens[1]) > (1 + (8 - n_bp))) return 0;

	// total promotions
	int w_promo = 0;
	w_promo += (popcount64(gd->nites[0]) > 2) ? popcount64(gd->nites[0]) - 2 : 0;
	w_promo += (popcount64(gd->bishops[0]) > 2) ? popcount64(gd->bishops[0]) - 2 : 0;
	w_promo += (popcount64(gd->rooks[0]) > 2) ? popcount64(gd->rooks[0]) - 2 : 0;
	w_promo += (popcount64(gd->queens[0]) > 1) ? popcount64(gd->queens[0]) - 1 : 0;

	int b_promo = 0;
	b_promo += (popcount64(gd->nites[1]) > 2) ? popcount64(gd->nites[1]) - 2 : 0;
	b_promo += (popcount64(gd->bishops[1]) > 2) ? popcount64(gd->bishops[1]) - 2 : 0;
	b_promo += (popcount64(gd->rooks[1]) > 2) ? popcount64(gd->rooks[1]) - 2 : 0;
	b_promo += (popcount64(gd->queens[1]) > 1) ? popcount64(gd->queens[1]) - 1 : 0;
	...
	else
	{
		...
		{	// sq in front must have own pawn
			if (!(PAWNSPUSH((ONE << ep_sq), gd->color ^ 1) & gd->pawns[gd->color ^ 1]))
			{
				gd->epsquare = 0;
			}
			else
			{	// sq behind must be empty
				if (((PAWNSPUSH((ONE << ep_sq), (gd->color))) & (all_w | all_b | gd->king[0] | gd->king[1])) != 0)
				{
					gd->epsquare = 0;
				}
			}
	...
	{
		if (!((gd->king[0] & (ONE << SQ_E1)) && (gd->rooks[0] & (ONE << SQ_H1))))
		{
	...
	{
		if (!((gd->king[1] & (ONE << SQ_E8)) && (gd->rooks[1] & (ONE << SQ_H8))))
		{
	...
	{
		if (!((gd->king[0] & (ONE << SQ_E1)) && (gd->rooks[0] & (ONE << SQ_A1))))
		{
	...
	{
		if (!((gd->king[1] & (ONE << SQ_E8)) && (gd->rooks[1] & (ONE << SQ_A8))))
		{
...
{
	// bodgy, this should work since all strings are stored as 256 length
	ptr += stringno * 256;
	if (*ptr == '\0') return 0;
	return ptr;
}
...
{
	char* ptr;
	char startpos[256] = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
	ptr = LoadFEN(gd, &startpos[0]);
	if (ptr == 0)
	{
...
int main()
{
...
#if TEST_ME_TO_DESTRUCTION
    // scruffy internal data structure for this code only
	GAMEDATA gamedata[1];
	// test startpos initialiser ....
	InitBoard(&gamedata[0]);
	...
	for (n_suite=1; n_suite<11; n_suite++)
	{
		...
		while (1)
		{
			...
			ptr = LoadFEN(gamedata, &tempepd[0]);
			n_epds++;
			if (ptr == 0)
			{
				n_fails++;
...
#endif
	while (1)
	{	// I prefer lazily sitting in a debugger loop to crashing back to windows
		int a = 1;
	}
	return 0;
}
